...
from fastapi import Request, Response # Keep Response for type hint, Request for adaptation
...
class ConsentManager:
    ...
    def get_consent_from_cookie(self, consent_type: str) -> Optional[Dict[str, Any]]:
          ...
          # This method is problematic outside a request context.
          # It's better to pass cookies directly. See get_consent_from_request_cookies.
          logger.warning("get_consent_from_cookie is deprecated. Use get_consent_from_request_cookies instead.")
          return None # Cannot access request context here

    def get_consent_from_request_cookies(self, consent_type: str, cookies: Dict[str, str]) -> Optional[Dict[str, Any]]:
          """Get consent data from cookie."""
          cookie_name = self.get_consent_cookie_name(consent_type)
          consent_cookie = cookies.get(cookie_name)
    ...
    def get_user_consent(self, consent_type: str, cookies: Dict[str, str]) -> Dict[str, Any]:
        """Get user consent data."""
        # Get from cookie
        consent = self.get_consent_from_request_cookies(consent_type, cookies)
        if consent:
            return consent
    ...
    def prepare_consent_cookie_params(self, consent_type: str, consent_data: Dict[str, Any], request: Request) -> Dict[str, Any]:
          """Prepare parameters needed to set the consent cookie on a FastAPI Response."""
          cookie_name = self.get_consent_cookie_name(consent_type)
          ...
          return {
              ...
              "httponly": True,
              "secure": is_secure,
              "samesite": 'lax' # FastAPI uses lowercase 'lax'
          }

    def record_consent(self, consent_type: str, preferences: Dict[str, bool],
                       request: Request) -> Dict[str, Any]:
          """Record user consent."""
          # This method now returns cookie parameters instead of modifying a response.
          # The FastAPI path operation will use these parameters to set the cookie.
          # Removed the optional response argument.
          ...
          return self.prepare_consent_cookie_params(consent_type, consent_data, request)

    def check_consent(self, consent_type: str, cookies: Dict[str, str], preference: Optional[str] = None) -> bool:
          """Check if user has given consent."""
          consent = self.get_user_consent(consent_type, cookies)
...
# ===== Flask Integration Removed =====
# TODO: Implement FastAPI path operations to serve legal documents and handle consent.
# - Create GET endpoints (e.g., /privacy-policy, /terms-of-service) that return HTMLResponse
#   using the generate_* functions and the legal_config instance.
# - Create a POST endpoint (e.g., /api/consent/{consent_type}) that accepts consent preferences
#   (likely via a Pydantic model), calls consent_manager.record_consent(..., request=request),
#   and sets the cookie on the FastAPI Response using the returned parameters.
