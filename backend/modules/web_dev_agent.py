        ...
        for tag in unclosed_tags:
            if tag in ['img', 'br', 'hr', 'input', 'meta', 'link']:
                ...
                if re.search(pattern, modified_content):
                    modified_content = re.sub(pattern, modified_content, replacement, count=1)
                    fixed = True
    ...
    def _optimize_image_references(self, directory_path: str) -> Dict[str, Any]:
        ...
        for file_path in html_files + css_files:
            try:
                ...
                # For HTML files, add width and height attributes to images
                if file_path.endswith('.html'):
                    # Add loading="lazy" for images below the fold
                    img_pattern = r'<img([^>]*)src=["\']((?!data:)[^"\']+)["\']([^>]*)>'
                    ...
                    def optimize_img_tag(match):
                        ...
                        # Add loading="lazy" if not present and not a critical image
                        if 'loading=' not in before_src + after_src and not any(x in src.lower() for x in ['logo', 'header', 'hero']):
                            after_src += ' loading="lazy"'
                            file_optimizations.append(f"Added lazy loading for {src}")
                        
                        # Add alt if missing
                        if 'alt=' not in before_src + after_src:
                            image_name = src.split('/')[-1].split('.')[0].replace('-', ' ').replace('_', ' ').title()
                            after_src += f' alt="{image_name}"'
                            file_optimizations.append(f"Added alt text for {src}")
                        
                        return f'<img{before_src}src="{src}"{after_src}>'
                    
                    modified_content = re.sub(img_pattern, optimize_img_tag, modified_content)
                
                # For CSS files, optimize background images
                if file_path.endswith('.css'):
                    # Add support for WebP with fallback
                    bg_pattern = r'background-image:\s*url\(["\']?([^"\')]+)["\']?\)'
                    
                    def optimize_bg_image(match):
                        image_url = match.group(1)
                        if image_url.lower().endswith(('.jpg', '.jpeg', '.png')) and 'webp' not in image_url:
                            file_optimizations.append(f"Added WebP suggestion for {image_url}")
                            return f"/* Consider converting to WebP: {image_url} */\n    {match.group(0)}"
                        return match.group(0)
                    
                    modified_content = re.sub(bg_pattern, optimize_bg_image, modified_content)
                ...
                if content != modified_content:
                    ...
                    optimization["files"][file_path] = file_optimizations
                    optimization["total_optimizations"] += len(file_optimizations)
                
            except Exception as e:
                if self.debug_mode:
                    print(f"Error optimizing image references in {file_path}: {str(e)}")
        
        return optimization
    
    def fix_google_sign_in(self, client_id: str, file_path: str) -> Dict[str, Any]:
        """
        Fix Google Sign-In implementation in a JavaScript file
        
        Args:
            client_id: Google OAuth client ID
            file_path: Path to the JS file implementing Google Sign-In
        ...
        try:
            ...
            # Check Google Sign-In initialization
            if 'google.accounts.id.initialize' in content:
                ...
                if re.search(client_id_pattern, content):
                    ...
                    old_client_id = re.search(client_id_pattern, content).group(1)
                    if old_client_id != client_id:
                        ...
                        report["fixed_issues"].append(f"Updated Google client ID from {old_client_id} to {client_id}")
                else:
                    report["remaining_issues"].append("Could not locate client_id in Google Sign-In initialization")
                
                # Ensure callback is properly defined
                callback_pattern = r'callback: (\w+)'
                ...
                if callback_match:
                    ...
                    if not re.search(rf'function\s+{callback_name}\s*\(', content):
                        # Add the missing callback function
                        callback_code = f"""
function {callback_name}(response) {{
  if (response && response.credential) {{
    // Process the credential
    console.log("ID Token received:", response.credential.substring(0, 10) + "...");
    
    // Here you would send the token to your backend for validation
    // Example:
    // fetch('/auth/google', {{
    //   method: 'POST',
...
"""
                        # Add before the initialize function
                        init_index = modified_content.find("google.accounts.id.initialize")
                        if init_index > 0:
                            # Find a good insertion point
                            insertion_point = modified_content.rfind("\n\n", 0, init_index)
                            if insertion_point > 0:
                                ...
                                report["fixed_issues"].append(f"Added missing callback function: {callback_name}")
                else:
                    # No callback defined
                    report["remaining_issues"].append("No callback function specified in Google Sign-In initialization")
                
                # Check for renderButton implementation
                if 'google.accounts.id.renderButton' not in content:
                    # Look for a container element
                    container_pattern = r'document\.getElementById\([\'"](\w+)[\'"]\)'
                    container_matches = re.findall(container_pattern, content)
                    
                    potential_containers = [c for c in container_matches if 'google' in c.lower() or 'sign' in c.lower() or 'auth' in c.lower() or 'login' in c.lower()]
                    
                    container_id = potential_containers[0] if potential_containers else 'google-signin-button'
                    
                    render_code = f"""
  // Render the Google Sign-In button
  const buttonContainer = document.getElementById('{container_id}');
  if (buttonContainer) {{
    google.accounts.id.renderButton(
      buttonContainer,
      {{ 
...
"""
                    # Add after the initialize call
                    init_pattern = r"(google\.accounts\.id\.initialize\s*\({[^}]*}\s*\);)"
                    if re.search(init_pattern, modified_content):
                        modified_content = re.sub(
                            init_pattern,
                            r"\1" + render_code,
                        ...
                        report["fixed_issues"].append(f"Added missing renderButton implementation for container '{container_id}'")
                    else:
                        report["remaining_issues"].append("Could not locate Google initialization code to add renderButton")
            else:
                # No Google Sign-In initialization found, need to add complete implementation
                report["remaining_issues"].append("No Google Sign-In implementation found - needs complete implementation")
            ...
            report["remaining_issues"].append(f"Error optimizing image references in {file_path}: {str(e)}")
